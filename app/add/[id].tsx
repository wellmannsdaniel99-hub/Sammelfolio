...
export default function AddItemScreen() {
  ...
  const save = () => {
    ...
    addItem({ item, quantity: parsedQty, buyPrice: parsedPrice, condition: item.kind === 'Karte' ? condition : undefined, purchaseDate, notes: notes.trim() || undefined });
  ...
  return <SafeAreaView style={styles.safe}><ScrollView contentContainerStyle={styles.container} keyboardShouldPersistTaps="handled">
    ...
    <View style={styles.summary}><View style={styles.row}><Text style={styles.key}>Aktueller Marktwert</Text><Text style={styles.val}>{euro(item.marketPrice)}</Text></View><View style={styles.row}><Text style={styles.key}>Sprache</Text><Text style={styles.val}>Deutsch</Text></View><View style={styles.row}><Text style={styles.key}>Typ</Text><Text style={styles.val}>{item.kind}</Text></View></View>
