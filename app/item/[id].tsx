...
export default function ItemDetailScreen() {
  ...
  const openCardmarket = () => {
    const query = [item.name, item.setName, item.productType === 'Top-Trainer-Box' ? (isGerman ? 'Top-Trainer-Box' : 'Elite Trainer Box') : item.productType].filter(Boolean).join(' ');
    Linking.openURL(`https://www.cardmarket.com/de/Pokemon/Products/Search?searchString=${encodeURIComponent(query)}`);
  };

  return <SafeAreaView style={styles.safe}><ScrollView contentContainerStyle={styles.container}>
    <Pressable onPress={() => router.back()}><Text style={styles.back}>‹ Zurück</Text></Pressable>
    <View style={styles.hero}><View style={styles.languageBadge}><Text style={styles.languageText}>{item.language ?? 'DE'} · {languageName(item.language)}</Text></View>{owned?.photoUri ? <Image source={{ uri: owned.photoUri }} style={styles.photo} /> : <ProductSilhouette type={item.productType} size={150} />}<Text style={styles.type}>{item.productType === 'Top-Trainer-Box' ? `${isGerman?'TTB':'ETB'} · ${isGerman?'TOP-TRAINER-BOX':'ELITE TRAINER BOX'}` : item.productType?.toUpperCase() ?? 'SEALED'}</Text></View>
    <Text style={styles.title}>{item.name}</Text><Text style={styles.subtitle}>{item.setName ?? item.subtitle}</Text>
    <View style={styles.market}><Text style={styles.label}>{isGerman ? 'SAMMELFOLIO-MARKTWERT' : 'PERSÖNLICHER MARKTWERT'}</Text><Text style={styles.value}>{marketPrice > 0 ? euro(marketPrice) : 'Noch offen'}</Text><Text style={styles.note}>{isGerman ? (latestDate ? `Zuletzt gepflegt am ${latestDate}` : 'Noch kein zentral gepflegter Tagespreis') : 'EN/JP werden derzeit nicht zentral von Sammelfolio bewertet.'}</Text></View>
    {owned && <View style={styles.owned}><View style={styles.ownedTop}><View><Text style={styles.label}>IN DEINER SAMMLUNG</Text><Text style={styles.ownedCount}>{owned.quantity}× vorhanden</Text></View><Text style={styles.ownedValue}>{euro(currentValue)}</Text></View><View style={styles.metrics}><View><Text style={styles.metricLabel}>INVESTIERT</Text><Text style={styles.metricValue}>{euro(invested)}</Text></View><View><Text style={styles.metricLabel}>GEWINN / VERLUST</Text><Text style={[styles.metricValue,gain>=0?styles.positive:styles.negative]}>{gain>=0?'+':''}{euro(gain)}</Text></View></View>{owned.purchaseDate ? <Text style={styles.ownedMeta}>Kaufdatum: {owned.purchaseDate}</Text> : null}{owned.notes ? <Text style={styles.ownedMeta}>Notiz: {owned.notes}</Text> : null}</View>}
    <View style={styles.infoCard}><View style={styles.row}><Text style={styles.key}>Produkttyp</Text><Text style={styles.val}>{item.productType === 'Top-Trainer-Box' ? (isGerman?'TTB':'ETB') : item.productType ?? 'Sealed'}</Text></View><View style={styles.row}><Text style={styles.key}>Set / Serie</Text><Text style={styles.val}>{item.setName ?? '–'}</Text></View><View style={styles.row}><Text style={styles.key}>Sprache</Text><Text style={styles.val}>{languageName(item.language)}</Text></View><View style={[styles.row,styles.lastRow]}><Text style={styles.key}>Bewertung</Text><Text style={styles.val}>{isGerman ? 'Sammelfolio' : 'Persönlich'}</Text></View></View>
    {owned && <Pressable style={styles.editButton} onPress={() => router.push(`/edit-item/${item.id}`)}><Text style={styles.editButtonText}>Produkt bearbeiten</Text></Pressable>}
    <Pressable style={styles.cardmarket} onPress={openCardmarket}><Text style={styles.cardmarketText}>Auf Cardmarket ansehen ↗</Text></Pressable>
    <Text style={styles.externalNote}>Externer Link zu Cardmarket. Sammelfolio ist nicht mit Cardmarket verbunden.</Text>
